...
def index(request):
    ...
    if request.user.is_authenticated:
        ...
        suggestions = User.objects.exclude(pk__in=followings).exclude(username=request.user.username).order_by("?")[:5]
...
def user_create_post(request,pk):
    if request.method == 'POST':
        ...
        post = Post(creater=request.user, content_text=text,title=title,offer_price=offer_price,categories=categories,tages_n=tages_n, content_image=pic,status=status,Product_Price=Product_Price,posts_type="user_post",doc=doc,vedio=vedio,)
        post.save()
        return redirect("index")
...
def notification(request,nid):
    ...
    return render(request,"notification.html",{'fr':fr,"inv":inv, 'crt' : crt,
        'crt_count' : crt_count,'pag':pag,"foll":foll,"suggestions":suggestions,"intrests":intrests})  
...
def wishlis(request,pk):
    ...
    return render(request,'wishlist.html',{'wsh':wsh,"suggestions":suggestions,"intrests":intrests, 'crt' : crt,
        'crt_count' : crt_count,
        "invc":inv.count(),  })
...
def topicpage(request,pk):
    ...
    return render(request,'topicpage.html',{'intr':intr,'crt' : crt,
        'crt_count' : crt_count,
        "invc":inv.count(),  "posts": posts,"post_count":post_count,"button_text":button_text,"user_followers":user_followers,"intrests":intrests, "suggestions": suggestions,   })
...
def sent_friend_request_download(request,pk,userid):
    ...
    frequest =friend_request(from_user=from_user,to_user=to_user,stat="following")
    frequest.save()
    return redirect("download_item",pk,userid)
...
def delete_frd_download(request,pid,pk):
    ...
    return redirect('download_item',pid,pk)
